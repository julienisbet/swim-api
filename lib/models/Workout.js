...
module.exports = class Workout {
    ...
    const { rows } = await pool.query(
      `
        ...
        COALESCE(
          ...
          LEFT JOIN (SELECT * FROM parts order by parts.order_num) as parts on swim_sets.id = parts.swim_set_id
  ...
  static async reorderSet(setId, updatedParts) {
    const parts = await Promise.all(
      updatedParts.map((part) =>
        Part.update(part.partId, { orderNum: part.newOrder })
      )
    );
    return parts;
  }
